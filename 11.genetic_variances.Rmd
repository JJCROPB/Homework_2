...
```{r, eval=FALSE}
wd="D:/Documents/1a-Spring 2022/AGRO 932-BIOMET GENETIC & PLANT BREED/Assignments/#2"

d <- read.csv("#2/soybean_half-sib_yield.csv")
#install.packages("tidyr")
library(tidyr)
df <- gather(d, key="Env", value="yield", 2:4)
#Checking the phenotypic data to avoid mistakes
head(df)
dim(df)
table(df$Env)
table(df$rep)
...
```{r, eval=FALSE}
##Visualize the data to check mean, variance, distribution (histograms).
library(ggplot2)
...
##data looks good, consistency between environments
#the model
#pijr= μ + fi + lj + fi×lj + br + eijr
#pijr is the yield value of the ith family evaluated in the jth environment in the rth replicate
#fi is the effect of the ith family
#lj is the effect of the jth environment
#fi×lj is the interaction bewteen family and environment
#br is the effect of the rth replication
```
...
summary(fit)
plot(fit)
##this is the anova that we want
summary(aov(fit))
##we get it
...
tail(cc)
table(cc$date)
#generating anova table to interpret it
summary(aov(fit))
##calculate vprogreny
##calculate H2
##H2 is 0.89 is very high


##HOMEWORK
##Is this H2 SUPPOSE TO BE CONSTANT, DECREASE OR INCREASE? WHY? HYPOTHESIS?
#CALCULATE VALUES OVER TIME FOR EACH DATE
#PLUG RESULTS OVER THE DAYS
##ACEPT OR REJECT HYPOTHESIS
##DATA COLLECTION CAN BE MISTAKE, DEPENDIENG ON THE TIME OF THE DAY, THE WEATHER, IF IT WAS SOME SHADES ETC.
```
